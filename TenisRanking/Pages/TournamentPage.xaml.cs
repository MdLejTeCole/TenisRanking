...
public sealed partial class TournamentPage : ExtendedPage
{
    ...
    private async void Name_LostFocus(object sender, RoutedEventArgs e)
    {
        ...
        if (tournament != null)
        {
            ...
            {
                await ShowInformationDialog($"Intnieje juz turniej zakończony turniej o takiej nazwie {tournament.Name}. Zmień nazwe turnieju na unikalną");
                return;
            }
            if (await ShowConfirmationDialog($"Intnieje juz turniej o nazwie {tournament.Name}. Czy chcesz go edytować?") == ContentDialogResult.Primary)
            {
                _isUpdate = true;
